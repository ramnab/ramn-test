...
echo """
Deploying blacklist AWS Connect widget
---------------------------------------
  config:          ${CONF}
  client:          ${CLIENT}
  env:             ${ENV}
...
aws cloudformation package --region ${DEPLOY_REGION} \
                           --template-file ${DIRECTORY}/resources/blacklist-widget.yml \
                           --s3-bucket ${LAMBDA_S3} \
                           --output-template-file deploy-blacklist-widget.yml

aws cloudformation deploy --region ${DEPLOY_REGION} \
                          --template-file deploy-blacklist-widget.yml \
                          --stack-name stCapita-CCM-${CLIENT}-${ENV}-Blacklist  \
...
rm deploy-blacklist-widget.yml
