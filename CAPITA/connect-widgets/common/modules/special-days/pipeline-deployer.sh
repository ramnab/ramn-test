...
echo """
Deploying special days AWS Connect widget
---------------------------------------
  config:          ${CONF}
  client:          ${CLIENT}
  env:             ${ENV}
...
aws cloudformation package --region ${DEPLOY_REGION} \
                           --template-file ${DIRECTORY}/resources/specialdays-widget.yml \
                           --s3-bucket ${LAMBDA_S3} \
                           --output-template-file deploy-specialdays-widget.yml

aws cloudformation deploy --region ${DEPLOY_REGION} \
                          --template-file deploy-specialdays-widget.yml \
                          --stack-name stCapita-CCM-${CLIENT}-${ENV}-SpecialDay  \
...
rm deploy-specialdays-widget.yml
