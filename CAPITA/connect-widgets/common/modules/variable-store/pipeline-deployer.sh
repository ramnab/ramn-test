...
echo """
Deploying variable-store AWS Connect widget
---------------------------------------
  config:          ${CONF}
  client:          ${CLIENT}
  env:             ${ENV}
...
aws cloudformation package --region ${DEPLOY_REGION} \
                           --template-file ${DIRECTORY}/resources/variablestore-widget.yml \
                           --s3-bucket ${LAMBDA_S3} \
                           --output-template-file deploy-variablestore-widget.yml

aws cloudformation deploy --region ${DEPLOY_REGION} \
                          --template-file deploy-variablestore-widget.yml \
                          --stack-name stCapita-CCM-${CLIENT}-${ENV}-VariableStore  \
...
rm deploy-variablestore-widget.yml
