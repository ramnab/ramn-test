...
Each entry/line is then published to the specified firehose
Config is a list of normalised fields that should be kept as a string
'''
...
def add_extra_metrics(report_json):

    if not report_json:
        return report_json

    for entry in report_json:
        logger.info(f"@republish|add_extra_metrics|entry={str(entry)}")
        avg_outbound_agent_interaction_time = entry.get("averageoutboundagentinteractiontime")
        if not avg_outbound_agent_interaction_time:
            avg_outbound_agent_interaction_time = 0

        contacts_handled_outbound = entry.get("contactshandledoutbound")
        if not contacts_handled_outbound:
            contacts_handled_outbound = 0

        avg_outbound_after_contact_work_time = entry.get("averageoutboundaftercontactworktime")
        if not avg_outbound_after_contact_work_time:
            avg_outbound_after_contact_work_time = 0

        logger.info(f"@republish|add_extra_metrics|avg_outbound_agent__interaction_time="
                    f"{avg_outbound_agent_interaction_time}")
        logger.info(f"@republish|add_extra_metrics|contacts_handled_outbound={contacts_handled_outbound}")
        logger.info(f"@republish|add_extra_metrics|avg_outbound_after_contact_work_time="
                    f"{avg_outbound_after_contact_work_time}")

        outbound_agent_interaction_time = avg_outbound_agent_interaction_time * contacts_handled_outbound
        outbound_agent_after_contact_work_time = avg_outbound_after_contact_work_time * contacts_handled_outbound

        logger.info(f"@republish|add_extra_metrics|outbound_agent_interaction_time="
                    f"{outbound_agent_interaction_time}")
        logger.info(f"@republish|add_extra_metrics|outbound_agent_after_contact_work_time="
                    f"{outbound_agent_after_contact_work_time}")

        entry['outboundagentinteractiontime'] = outbound_agent_interaction_time
        entry['outboundagentaftercontactworktime'] = outbound_agent_after_contact_work_time
        logger.info(f"@republish|add_extra_metrics|new entry={str(entry)}")
