...
if [ -z "$1" ] || [ -z "$2" ]; then
    echo "Usage: update-customer-queue-intervals-trigger.sh <CLIENT> <ENV>"
    exit
fi
...
echo "Updating queue interval report trigger for client $CLIENT, env $ENV_UPPER"
...
aws lambda invoke --function-name lmbMiReportingModder-ccm-$ENV_UPPER \
  ...
  \"lambda\": \"lmbMIQueueInterval-ccm-${ENV_UPPER}\"
}" result.txt
