...
if [ -z "$1" ] || [ -z "$2" ]; then
    echo "Usage: update-customer-agent-daily-fh.sh <CLIENT> <ENV>"
    exit
fi
...
echo "Updating agent daily firehose for client $CLIENT, env $ENV_UPPER"
...
echo "Adding tags to firehose kfh-ccm-ai-daily-${ENV_LOWER}"
