...
if [ -z "$1" ] || [ -z "$2" ]; then
    echo "Usage: update-customer-queue-daily-fh.sh <CLIENT> <ENV>"
    exit
fi
...
echo "Updating queue daily firehose for client $CLIENT, env $ENV_UPPER"
...
echo "Adding tags to firehose kfh-ccm-qi-daily-${ENV_LOWER}"
