...
# ARG_OPTIONAL_SINGLE([department],[d],[Department],[ccm])
# ARG_POSITIONAL_SINGLE([client],[Client short name])
# ARG_POSITIONAL_SINGLE([env],[Environment tag])
# ARG_POSITIONAL_SINGLE([module],[Specific module to deploy],[all])
# ARG_HELP([Deploy MI solution to client account])
# ARGBASH_GO()
...
print_help ()
{
	printf '%s\n' "Deploy MI solution to client account"
	printf 'Usage: %s [-r|--region <arg>] [-d|--department <arg>] [-h|--help] <client> <env> [<module>]\n' "$0"
...
parse_commandline ()
{
	...
	do
		...
		case "$_key" in
			...
			# Therefore, we expect the --region or -r value.
			# so we watch for --region and -r.
			...
			# Therefore, we expect --region=value, so we watch for --region=*
			# For whatever we get, we strip '--region=' using the ${var##--region=} notation
			...
			-r*)
				_arg_region="${_key##-r}"
				;;
			# See the comment of option '--region' to see what's going on here - principle is the same.
			-d|--department)
				test $# -lt 2 && die "Missing value for the optional argument '$_key'." 1
				_arg_department="$2"
				shift
...
{
	_required_args_string="'client' and 'env'"
...
assign_positional_args ()
{
	...
	_positional_names=('_arg_client' '_arg_env' '_arg_module' )
